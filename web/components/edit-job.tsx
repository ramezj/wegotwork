...
export function EditJobCard({
  ...
  const [currentCategoryId, setCurrentCategoryId] = useState<null | string>(
    job.categoryId === null ? "none" : job.categoryId,
  );
    ...
    <>
      <div className="flex flex-col gap-4">
          ...
          <CardContent>
            <form onSubmit={EditTheJob} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  required
                  className="bg-accent border border-dashed text-white font-medium text-sm rounded-none"
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger
                    value={current.type}
                    className="bg-accent rounded-none border border-dashed border-white/20 text-white font-medium text-sm"
                    defaultValue={current.type}
                  >
                    <SelectValue>{formatJobType(current.type)}</SelectValue>
                  </SelectTrigger>
                  <SelectContent className="bg-theme rounded-none border border-dashed text-white font-medium">
                    <SelectGroup className="space-y-1">
                      <SelectItem className="rounded-none" value="fulltime">
                        Full Time
                      </SelectItem>
                      <SelectItem className="rounded-none" value="parttime">
                        Part Time
                      </SelectItem>
                      <SelectItem className="rounded-none" value="internship">
                        Internship
                      </SelectItem>
                      <SelectItem className="rounded-none" value="contract">
                        Contract
                      </SelectItem>
                    </SelectGroup>
                  </SelectContent>
              ...
              <div className="gap-2">
                <Button
                  ...
                  variant={"default"}
                  disabled={loading}
                  className="px-4 font-medium rounded-none"
                >
                  {loading ? (
        ...
        <Card className="w-full rounded-none bg-theme border border-dashed">
          <CardHeader className="">
            <div className="flex justify-between items-center w-full">
              <CardTitle className="text-white font-medium">Location</CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            <form onSubmit={EditTheJob} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="flex flex-row gap-2">
                  <Input
                    ...
                    value={current.country == null ? "" : current.country}
                    onChange={(e) => {
                      setCurrent((previous) => ({
                        ...previous,
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={current.remote === true ? "true" : "false"}
                  onValueChange={(e) => {
                    setCurrent((previous) => ({
                      ...previous,
                      remote: e === "true",
                ...
                >
                  <SelectTrigger
                    value={current.remote === true ? "true" : "false"}
                    className="bg-accent rounded-none border border-dashed border-white/20 text-white font-medium text-sm"
                    defaultValue={current.remote === true ? "true" : "false"}
                  >
                    <SelectValue>
                      {current.remote === true
                        ? "Remote Friendly"
                  ...
                  <SelectContent className="bg-theme rounded-none border border-dashed text-white font-medium">
                    <SelectGroup className="space-y-1">
                      <SelectItem className="rounded-none" value="false">
                        No Remote
                      </SelectItem>
                      <SelectItem className="rounded-none" value="true">
                        Remote Friendly
                      </SelectItem>
                    </SelectGroup>
                  </SelectContent>
            ...
            <form onSubmit={EditTheJob} className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  value={current.content as string}
                  onChange={(e) => {
                    setCurrent((previous) => ({
                      ...previous,
