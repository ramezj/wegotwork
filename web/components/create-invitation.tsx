...
export function CreateUserInvitation({ organizationId} : { organizationId: string}) {
    ...
    return (
      ...
      <CardContent className="">
        ...
        <Input className="rounded-none bg-accent border border-dashed hover:border-white/50 active:border-white/50 focus:border-white/50 text-white font-medium text-base" type="email" required placeholder="Enter member's email address" value={email as string} onChange={((e) => {setEmail(e.target.value)})} />
