      ...
      <div className="w-full bg-theme flex border border-foreground/20 border-dashed hover:border-white/30 duration-200 rounded-none items-center pt-3 pb-3 cursor-pointer">
        <div className="mx-5 my-3 flex flex-col items-start text-left">
          ...
          <div className="mt-2 flex gap-2">
            <Badge
              variant={"outline"}
              className="rounded-none border-dashed border-white/20 bg-accent text-white border font-medium hover:bg-accent"
            >
              {formatJobType(job.type as Type)}
            </Badge>
            {job.categoryId === null ? (
              <></>
            ) : (
        ...
        <div className="ml-auto mr-5">
          ...
          <div className="sm:block hidden">
            <Button
              className="border rounded-none font-bold"
              variant={"default"}
            >
              View
              <ArrowUpRight className="size-4" />
...
export function LandingPageJobCard({
  title,
  type,
  location,
  age,
...
export function JobCardForApplicants({
  job,
  applicants,
}: {
  job: Job;
