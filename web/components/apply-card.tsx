...
export default function ApplyCard({ jobId }: { jobId: string}) {
    ...
    return (
        <>
            ...
            <Input className="bg-white border border-black rounded-none text-black font-medium text-base" required placeholder="Enter your full name" value={name} onChange={((e) => {setName(e.target.value)})} />
            </div>
            <div className="space-y-2">
            <Label htmlFor="name" className="text-black font-extrabold">Email address</Label>
            <Input className="bg-white border border-black rounded-none text-black font-medium text-base" required placeholder="Enter your email address" value={emailAddress} onChange={((e) => {setEmailAddress(e.target.value)})} />
            </div>
            <div className="space-y-2">
            ...
            <div>
            <Input className="bg-white file:text-black file:font-bold border border-black rounded-none text-black font-medium text-base" required type="file" id="file" name="file" accept=".pdf" onChange={handleFileChange} />
            </div>
            </div>
            <div className="space-y-2"> 
            <Label htmlFor="motivation" className="text-black font-extrabold">Motivation letter</Label>
            <Textarea className="bg-white border border-black rounded-none text-black font-medium text-base" required value={motivation} onChange={((e) => {setMotivation(e.target.value)})} placeholder="Share why you're a great fit." />
            </div>
            {
                loading 
                ?  
