      ...
      <div className="flex flex-col w-full lg:w-[60%] items-start">
        ...
        <div className="flex sm:flex-row flex-col gap-4 w-full pt-2 justify-center">
          <div className="w-full">
            <Input
              placeholder="Search"
              className="bg-theme font-bold rounded-none border-dashed border"
            />
          </div>
          <div className="w-full">
            <Select
          ...
          {organization.categories
            .map((category) => ({
              ...
              jobs: category.jobs.filter((job) =>
                jobs.some((filteredJob) => filteredJob.id === job.id),
              ),
            }))
            ...
            .map((category: CategoryWithJobs, index) => {
              return (
                <div
                  ...
                  key={category.id}
                >
                  <motion.h1 className="font-bold text-white text-2xl pb-2 pt-2">
                    {category.name}
                  </motion.h1>
