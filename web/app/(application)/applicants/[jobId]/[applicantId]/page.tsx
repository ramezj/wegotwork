...
export default async function Page({
  ...
  params: Promise<{ organization: string; jobId: string; applicantId: string }>;
}) {
  ...
  return (
    <>
      <div className="flex items-center justify-between w-full">
        <h1 className="font-medium text-3xl text-black tracking-tight">
          {applicant?.applicant?.name}
        </h1>
        {/* <CreateJob id={await((await (params)).organization)} buttonSize="sm" buttonColor="white" /> */}
      </div>
      <Button asChild>
        <Link href={applicant?.url!}>CV</Link>
      </Button>
    </>
  );
}
