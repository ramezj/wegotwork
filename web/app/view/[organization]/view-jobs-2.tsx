...
export function ViewJobs2({
  ...
  const [originalJobs, setOriginalJobs] = useState<Array<Job>>(
    organization.jobs,
  );
  ...
  return (
    <div className="w-full flex flex-col items-center text-center p-4 space-y-1 overflow-hidden">
      <h1 className="font-medium text-white text-4xl pt-6">
        {organization?.name}
      </h1>
      {organization.description ? (
        <>
          <Balancer>
            <p className="max-w-3xl text-sm pt-3 pb-3 text-white font-bold">
              {organization?.description}
            </p>
          </Balancer>
        </>
          ...
          >
            ...
            <SelectContent className="bg-theme border-white/20 border rounded-none text-white font-bold mt-1">
              <SelectGroup className="space-y-1">
                <SelectItem className="rounded-none" key={"All"} value="All">
                  All Locations
                </SelectItem>
            ...
            >
              <SelectValue placeholder="All Employment">
                {selectedEmploymentType === "All"
                  ? "All Employment"
                  : formatJobType(selectedEmploymentType as Type)}
              </SelectValue>
            </SelectTrigger>
            <SelectContent className="bg-theme border-white/20 border rounded-none text-white font-bold mt-1">
              <SelectGroup className="space-y-1">
      ...
      <div className="flex flex-col gap-4 lg:w-1/2 w-full pt-6">
        {jobs
          .filter((job) => job.categoryId === null)
          .map((job: Job, index) => {
            return (
          ...
          .map((category: CategoryWithJobs, index) => {
            return (
              <div className="text-left flex flex-col gap-4" key={category.id}>
                <motion.h1 className="font-medium text-white text-2xl pb-2 pt-2">
                  {category.name}
                </motion.h1>
                  ...
                  return (
                    <motion.div key={job.id} aria-label="Job">
                      <div className="flex items-center justify-between border-b py-4 cursor-pointer">
                        <a
                          href={`/view/${organization.slug}/${job.id}`}
